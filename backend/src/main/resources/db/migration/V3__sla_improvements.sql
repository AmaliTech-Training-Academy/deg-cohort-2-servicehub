...
-- Drop the priority-only unique constraint before inserting rows that repeat priorities across categories
...
-- Replace with composite (category, priority) unique constraint
DO $$
BEGIN
    IF NOT EXISTS (
